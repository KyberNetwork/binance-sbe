...
func (c *ClientWs) read() {
	...
	for {
		...
		// name="WebSocketResponse" id="50" description="Message wrapper for WebSocket API."
		if header.TemplateId != 50 {
			continue
		}

		var wsData types.WsWrapperSbe
		if err := wsData.Decode(marshaller, reader); err != nil {
			continue
		}

		if call := c.pending.get(wsData.Id); call != nil {
			call.response = wsData
			call.done <- nil
			close(call.done)
			c.pending.remove(wsData.Id)
		}
	}
}
